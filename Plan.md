...
| G | 実証モデルA: リコール検知 | 苦情DB全件+リコール台帳全件で、事前登録プロトコル(docs/140)による時系列分割評価を実行 | [docs/140](docs/140_recall_detection_protocol.md)(事前登録) + [docs/141](docs/141_recall_detection_results.md)(結果) + scripts×2 + [結果ログ](data/recall_detection_results.tsv) | **完了(v1)。事前登録基準(prec≥0.5∧rec≥0.3)は未達(0.39/0.17)→「本特徴量セットでは実用シグナル不成立」とそのまま報告。ただし信号は実在(PR-AUC 0.315=無情報の2.9倍、ROC 0.783、手作りルール比で大幅改善)。最大の改善余地はモデル名マッチ率21%。v2はプロトコル改訂の事前コミット後に実行** |
