...
**ビジネスモデル定義(2026-07-07、[docs/138](docs/138_business_model_definition.md)): 3層構造。第1層(唯一の収益線)= 状態説明機能を組み込んだEPS製品仕様をOEMにRFQ差別化+診断コンテンツNREとして売る。第2層= 市場シグナル監視は内部投資(第1層の信用の裏付け)。第3層= assessmentはprogram付帯NREのみ。事業のKill条件はBM-KQ1〜4(RFQ要求の実在、採点影響の有無、NRE余地、サプライヤ内利用)。**
...
| M | 高率モデル×公開記録の突き合わせ | 答え合わせ3例目: 群シグナル上位が公開の不具合記録と対応するか | [docs/151](docs/151_high_rate_model_crosscheck.md) | **完了・成立。Corsa=公知のEPSコラム故障ファミリー+リコール複数(断続的アシスト喪失=本研究のターゲットケースが3つ目の独立市場で確認)、バン勢=用途起因の摩耗。「群監視は何かがあるまでを示し、欠陥か使い方かはモードを見る」教訓が3例目でも成立。能動的な公開検証はここで打ち止め、以後は観測台帳のみ** |
