...
> **最新の結論は [docs/224](docs/224_current_conclusion.md)。復帰時はまずそこを読む。**
>
> 2026-08-16〜30に事前登録つき検証を12件実行した。**結論は非対称である。**
> - **立証できた**: 外部観測は**6系統すべて盲目**(6番目は制度的——車検の機能系項目に兆候の区分が無い)。
>   個体・セッション基準は必須(実車でも床が1.3倍ひらく)。実車の観測床 **0.19〜0.25 m/s²**、
>   0.10 m/s² に必要な窓 **2.0〜5.0秒**
> - **落ちた**: 機体をまたぐcapability値、早期警報、不安定さの先行、故障の場所特定、動作点正規化(実車で0/4)
> - **到達できない**: 断続故障そのもの。**公開データが1件も存在しない**
...
> **上の読み順は2026-08-23までのものである。2026-08-30以降は [docs/224](docs/224_current_conclusion.md) を先に読み、
> そこから必要な文書へ降りる。**
