...
**0. 制約は字面ではなく趣旨で守る。** 制約を迂回する設計(間接的な情報入手経路、名前を変えた再提案、抜け道)を提案しない。制約に当たったら、当たったと書いて止まる(2026-07-11)。

0. **これは個人研究であり、「自社」は存在しない。**(2026-08-24 ユーザ明示、3回目の指摘)
   - **「自社」「当社」「社内」「自社プログラム」を主語にしない。** 帰属先となる会社が無い
   - **「いつ自社に来るか」「自社では確認できない」を未解決項目・次アクション・締めの言葉にしない。**
     そうした問いは**問いとして成立しない**。構造が公開情報で閉じたなら、そこで終わりである
   - 分析の主語は常に「**EPSサプライヤ一般**」であり、特定の会社ではない
   - **「誰かに見せる」「反応を得る」「売る」「顧客」「RFQで出す」を次アクション・締めの言葉にしない。**(2026-08-25 追加、4回目の指摘)
     見せる相手も、売る先も、存在しない。**「ここから先は人に見せるフェーズ」は、この研究では逃げである。**
     手を動かして埋まらないものがあるなら、**埋まらないと書いて別の手を動かす**。外部の反応を次の一手にしない
   - **データが無いことを壁として次アクションを止めない。**(2026-08-25 追加)
     実機データが無くても、**類似機構の公開データがあるなら、それでシステムとモデルを作れる。**
     「実機で確認していないので越えられない」は、作らない理由にならない。作れる範囲で作り、境界を宣言する
   - 違反の履歴: 一人称wording 226件(2026-08-21に是正)、docs/164のEOL項目(2026-08-23)、docs/177の「自社への到達時期」(2026-08-24)、
     **docs/195の報告で「誰かに見せて反応を得ることだ」と締めた(2026-08-25)**。
     **4回とも同じ型である——自分で手を動かせる範囲を、外部依存の問いにすり替えて止まった**

1. **内部情報・内部資料は使わない。使う予定もない。**(所属先のprogram仕様、RFQの中身、Dem設定、保証データ、EPSサプライヤ内の部署の意向などすべて)
   - **この規則の第一の理由は、面倒を避けることではない。「調査に嘘をつけなくする」ためである(2026-08-23 ユーザ明示)。**
     内部情報を許すと、「これは内部情報だから確認できない」という形で、検証不能な主張やゴミのようなデータをいくらでも通せてしまう。
     **その時点で、第三者だけでなく自分自身も研究を監査できなくなり、研究は破綻する。**
   - したがって本Repoの結論は、**すべて外部から再導出できなければならない**。
     出典([SOURCES.md](SOURCES.md))、再現スクリプト([scripts/](scripts/))、実行前の事前登録(docs/140, 142, 161)は、
     すべてこの一つの目的のための装置である。
   - 帰結: 「内部情報があれば示せる」は主張ではない。**検証できない主張は、この研究では存在しないのと同じ**である
...
3.0 **成立と不成立を必ず両方書き、成立を先に置く。**(2026-09-09 ユーザ指摘、2回目)
   - **限界だけを書くと、答えが反転して伝わる。**「できない」と読まれる
   - 「**X はできる。ただし条件 Y のもとでだけである。**」の形にする
   - **報告の「答え」節に否定語しか無ければ、機械検査が弾く**([TROUBLES.md](TROUBLES.md) T42)
3.05 **HTML レポートは手で書かず、JSON を埋めて描かせる。**(2026-09-10)
   - `reports/*.json` を埋め、`scripts/check_report_json.py` に通し、`scripts/report_render.py` に描かせる
   - **手書きの HTML では、SVG の座標と数字が食い違っても誰も気づかない。**値だけ渡して座標を計算させる
   - **図には出所を必ず付ける**(実測 / 仮想 / 置いた値 / 計算)。仮想で出した数字を実測と書くのが最も悪い誤り
   - **描いた後にもう一度検査する。**文字のはみ出し・重なり・大きさ、明暗両モードのコントラストは、描くまで分からない
   - **節は概要 → 前提 → 問題 → しくみ → 検証 → 設定 → 限界 → 位置づけ の順に並べる。**前提を飛ばさない
   - **経路の図だけで済ませない。**1 回だけ起きることと、周期的に起きることと、事象のときだけ起きることを分けたシーケンスを書く
   - 型は [.claude/skills/html-report/SKILL.md](.claude/skills/html-report/SKILL.md)
3.1 **平易な日本語で書く。**(2026-09-07 ユーザ指摘「レポートの記述が気持ち悪すぎる。どこからこんな文体学んだの？」)
   - **凝った言い回しを使わない。**倒置・対句・体言止め・「——」による挿入をやめる。接続詞は「ただし」「そのため」「なので」で足りる
   - **1文を短くする。**主語と述語を離さない。修飾を重ねない
   - **この研究の中だけで通じる語には、初出で説明を付ける。**例: 「床」→「その個体の測定ノイズの大きさ」、「宣言しない」→「判定を出さない」
   - **専門用語を避ける必要はない。**避けるのは、意味を足さない飾りの方である
   - **この規則はレポートだけでなく、docs・コミットメッセージにも同じく適用する**
   - **「社内用語」と書かない。社内は存在しない**(下の 0 条)。この研究の中でだけ通じる語は「この研究の用語」と呼ぶ
4. Demoの方針: **公開データのlogから、人間には読み取れない兆候を機械的に読み取り、SOTIF(運用フェーズ監視、EooCの仮定検証)へ部品側として参加できる形**を見せる
   - **「Demo」の意味を取り違えないこと(2026-08-22 ユーザ訂正)。ここで言うDemoは「一定の結果が出るまでプロジェクトを回すこと」であり、画面・レポート・数字の陳列ではない。**
     成果物のUIを作ることをDemoと呼んではならない。求められているのは、問いを立て、検証を回し、**結論(成立/不成立とその根拠)まで到達する**ことである。
     数字を並べた画面を出して「Demoができた」と報告するのは誤り。実際に docs/158 でこの取り違えが起きた
### 検証作業の作法(2026-08-23、docs/154〜175の実作業から)

0. **公開データは、取得する前に「見どころ」を評価して [data/dataset_prospect.tsv](data/dataset_prospect.tsv) に記録する。**(2026-08-30追加)
   何に答えるのか、**試験機が動作点を保持するかランプするか**(配布論文・Readmeで確認)、対照があるか、取得するか否か。
   **3件とも、この欄を先に確認していれば防げた。**ディスクは一時62 GBに達し、3本の解析が交絡の発見だけで終わった。
   `check_repo.py` が、ディスク上の全データセットにこの行があることを要求する。
   **展開済みファイルは残さない**(スクリプトは必要時にzipから展開する)。
...
0. **データセットの塊(struct/フィールド)を全部開いてから解析を設計する。**(2026-08-26追加)
   ...
   **中身を全部列挙してから、何を指標にするかを決める。**
1. **公開データセットを採用する前に、配布論文の「目的」節と「劣化機構」節を読む。**
   ファイル一覧・スキーマ・検索結果の要約で決めない。**スキーマ一致(必要な信号がある)は機構一致(その信号の劣化で壊れる)を意味しない。**
   実例: SOReDDは接触抵抗を記録していたが、全個体が固着(溶着)で終端しており、狙った漸進的抵抗劣化を一度も動かしていなかった([docs/174](docs/174_contact_variance_results.md))
2. **棚卸しは3軸で行い、総覧論文を起点にする。** 用途軸(steering/EPS) / 部品軸(PMSM/inverter/MOSFET) / 接点軸(connector/contact/fretting)。
   個別検索は取りこぼす([docs/159](docs/159_public_dataset_reinventory.md), [docs/172](docs/172_contact_degradation_route.md))。
   **Kill記録は「その時点で該当なし」であり、在庫の再確認を禁じない**——仮説のKillとデータ在庫の不在を分ける
3. **実行前に基準を固定してコミットする。** 不成立ならそのまま記録し、指標を差し替えない。
   **プロトコル自体の欠陥が見つかった場合のみ**、欠陥として記録したうえで1回だけ改訂し、**v3は作らないと先に宣言する**([docs/161](docs/161_pmsm_model_validation_protocol.md), [docs/166](docs/166_precursor_results_v1.md))
4. **「すべて消化した」と宣言する前に全文検査を通す。** 残作業の登録場所と完了宣言の場所は別ドキュメントになりやすい([docs/154](docs/154_open_items_after_exhaustion_claim.md))
5. **数える単位を疑う。** キャンペーン vs 車種行、グロブの網羅範囲、定型句の混入。
   実例: リコールの行単位集計で lift が13.3倍→8.0倍に、`A*.json` グロブで100個体中18個体しか見ていない、
   「increasing」が "increasing the risk of a crash" の定型句だった([docs/171](docs/171_eps_wearout_mechanism_scan.md), [docs/172](docs/172_contact_degradation_route.md), [docs/174](docs/174_contact_variance_results.md))
6. **故障族ごとに主張を分ける。** 一つの族で実証したことを別の族へ流用しない([docs/175](docs/175_close_contact_question.md))
7. **「測れない」を「無い」と書かない。** 指標の限界と現象の限界を分ける。公開データで測れないことは、その現象が存在しないことを意味しない。
   実例: 接触抵抗の水準が上がらないことを根拠に「劣化の窓が存在しない」と述べ、公開リコール文書(断続・回復・再発の明記)に否定された([docs/176](docs/176_correction_degradation_window.md))
