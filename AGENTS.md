...
6. **ビジネス判断は外から組み立てる**: 支払い意思・市場成立の判断を「内部情報がないと分からない」で止めない(それは判断の放棄であり三流)。競合の製品行動(継続投資・量産開発は支払い意思の最も硬い証拠)、買収の事実、規制の時間表、過去の先例の類型(法規化でタダ化した機能/有償化に成功した機能)、価格と工数の桁感——**公開の構造証拠で最も確からしい判断を下し、必ず誤り条件(何が観測されたらこの判断は間違いか)を添える**
