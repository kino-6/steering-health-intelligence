...
> 操舵系でpredictive diagnosticsの対象にできるstateを定義し、それをpredictive maintenance action、vehicle health output、diagnostic triage、unnecessary replacement avoidance、recurrence monitoringへつなげる。
...
現時点で、EPSサプライヤが初期に商品化してよい候補は「交換時期予測」単体ではない。
`steering predictive diagnostics` として、どのstateを予測対象にし、どのpredictive maintenance actionへつなげるかを定義することである。
...
- 代わりに、操舵系としてpredictive diagnosticsの対象にできるstateがあるかを見る
...
これは、EPS寿命予測ではない。
しかし「高負荷操舵が続き、熱保護に近い状態に入った / 入りそう」というpredictive stateなら、predictive maintenance action、diagnostic triage、unnecessary replacement avoidance、顧客説明に使える可能性がある。
...
これは、操舵系predictive stateとしてかなり強い。
...
予測価値は、EPS単体failure predictionではなく、vehicle health output、diagnostic triage、unnecessary replacement avoidance、責任境界、サービス説明にある。
...
これは、vehicle health側で強いpredictive stateになりやすい。
操舵異常だけでなく複数ECU症状が同時に出る場合、EPS内部ではなく電源 / harness / networkを先に見る、というdiagnostic triage valueがある。
...
ただし、熱環境、thermal protection、warning有無、assist reductionの関係を、predictive diagnosticsのstateとして考える材料になる。
...
これは予測診断の直接証拠ではない。
しかし、操舵系でどの状態をDTC、警告、fallback、degraded stateとして扱うかを、predictive diagnostics / vehicle healthへ翻訳する土台になる。
...
6. 特定hardware / thermal exposure由来のpredictive state

理由は、1から4は、EPS交換時期予測へ飛ばさなくても、predictive maintenance action、diagnostic triage、unnecessary replacement avoidance、顧客説明に落ちやすいからである。
...
> EPS / SbWサプライヤが、操舵系predictive stateを定義し、既存DTC、freeze frame、extended data、制御制限、電圧、温度、通信context、発生頻度、修理結果との関係を整理する短期assessment。

成果物は次に絞る。

1. 操舵系predictive state候補表
2. 各状態に対して、既存診断で見えるもの / 見えないもの
3. predictive maintenance action、vehicle health output、diagnostic triage、unnecessary replacement avoidance、顧客説明のどれに使えるか
...
ただし、最初の商材はEPS交換日予測単体ではなく、操舵系predictive diagnosticsを定義し、predictive maintenance actionやvehicle health outputへ変換するreadiness確認に置く。

Proceed条件:

- 診断企画が、熱保護、電圧、通信、DTC履歴、fallbackのうち3つ以上をpredictive stateとして扱える
- 品質改善またはサービス技術が、unnecessary replacement avoidanceまたはdiagnostic triageに使えると言える
- 既存DTC / freeze frame / extended dataで説明できる範囲と、追加で必要な範囲を分けられる
- repair feedback loopで、predictive stateが本当に整備行動に役立ったかを検証できる

Hold条件:

- predictive stateは見えるが、既存診断や既存サービス資料と同じ表になる
...
- 熱、電圧、通信、DTC履歴のpredictive stateが、既存診断説明と区別できない
...
|---|---|---:|---|
| 操舵系にpredictive diagnostics対象stateはあるか | Yes。熱保護、低/高電圧、過温度、通信値異常、複合電気症状、DTC履歴が公開資料に出る。 | High | 候補表と優先順位に反映 |
| これはEPS寿命予測か | No。初期候補は寿命や交換日単体ではなく、predictive maintenance action、diagnostic triage、unnecessary replacement avoidance、顧客説明に効くsteering predictive stateである。 | High | 結論と判定に反映 |
...
> Boschがpredictive diagnostics / predictive maintenance / vehicle healthを明確に打ち出しているため、操舵系でもsteering predictive stateを定義する価値がある。初期候補は、熱保護に近い状態、低/高電圧や過温度によるreduced assist、外部signalや通信値異常、電気接続やnetwork由来の複合症状、DTC履歴や再発間隔である。これらは、交換時期予測単体ではなく、predictive maintenance action、diagnostic triage、unnecessary replacement avoidance、顧客説明に使えるかを確認する。
...
> steering predictive stateを定義すれば、安全保証、root cause断定、保証費削減ができる。
...
1. 熱、電圧、通信、DTC履歴、fallbackのうち、steering predictive diagnosticsの対象にできるstateはどれか
2. それは既存DTC / freeze frame / extended data / service note / repair resultで、どこまでpredictive diagnosticsとして説明または検証できるか
