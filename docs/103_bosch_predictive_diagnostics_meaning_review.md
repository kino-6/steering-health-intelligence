...
> EPSサプライヤが、Bosch型のvehicle health / predictive diagnostics / fleet maintenance基盤に接続されるとき、操舵系のどの負荷特徴、診断特徴、limit / fallback context、DTC / freeze frame解釈を渡せるかを整理する。
...
> OEMやfleet向けの予測診断基盤に、操舵系をどう載せるかを決めるため、EPSサプライヤ側で出せる特徴量、診断値、説明境界、禁止主張、feedback loop条件を整理する短期assessment。
...
- 代わりに、操舵系のcomponent-specific load and diagnostic featuresを定義できるかを見る
...
この記述は、Repoにとって最も重要である。
EPSサプライヤが問うべきことは、「自分たちもfleet platformを売れるか」ではない。
操舵系で、component-specific load featureやdiagnostic featureとして何を定義できるかである。
...
次のような、fleet / OEM predictive diagnostics基盤へ渡すための操舵系contextである。

| 予測または先読み対象 | EPSサプライヤ側の材料 | 使い道 | 言ってはいけないこと |
|---|---|---|---|
| 操舵系の負荷状態class | 操舵角、操舵速度、トルク要求、モータ電流、温度、電圧、路面外乱context | fleet / OEM側で点検優先度や診断読み順の材料にする | EPS寿命や交換時期を断定する |
| limit / fallbackに近いcontext | assist limit、thermal limit、voltage drop、communication degradation、redundancy degraded | vehicle health側で注意すべき状態として扱う | 安全保証やroot cause断定をする |
| diagnostic feature | DTC、freeze frame、extended data、software / calibration ID、発生頻度、再発間隔 | predictive diagnostics側のfeature候補や説明材料にする | DTCだけで故障時期が分かると言う |
| behavior-based failure predictionへの入力 | 使用条件、操舵負荷、stop-start、低速高操舵、路面・振動exposure | Bosch型のmulti-source modelへ操舵系featureとして渡せるか確認する | 公開proxyだけで予測精度が出ると言う |
| repair feedback loop | 整備結果、交換有無、再発有無、作業時間 | model validationやfeature改善に使う | EPSサプライヤ単独で閉じたloopがあると断定する |

## ビジネス仮説

Boschの2026年発表を踏まえると、Repoのビジネス仮説は次へ補正できる。

> EPSサプライヤは、OEM / fleet / platform側のpredictive diagnosticsに操舵系を載せるための、feature readiness、診断境界、禁止主張、feedback loop条件を整理する短期assessmentを提供できる可能性がある。
...
1. 操舵系のcomponent-specific load feature候補表
2. DTC / freeze frame / extended data / software IDを予測診断側で使う場合の意味と限界
3. limit / fallback / degradation contextをvehicle health側へ渡す場合の説明境界
4. OEM / fleet / platform側から必要なfeedback loop条件
...
最初は、OEM / fleet / platform側のpredictive diagnosticsに接続するための社内readiness確認として扱う。
...
Boschの`component-specific load and diagnostic features`という考え方に合わせるなら、EPSサプライヤは「操舵系にとって意味のあるload featureとdiagnostic featureは何か」を定義する側に回れる可能性がある。
...
2. OEMやfleet platformが、steering-specific featureをsupplierから受け取りたいか
3. EPSサプライヤ内に、予測診断へ渡せるload feature / diagnostic featureの候補が既にあるか
...
- 診断企画または品質改善が、操舵系のcomponent-specific load feature候補を3つ以上挙げられる
- DTC / freeze frame / extended data / software IDを、予測診断側のfeatureまたは説明材料として整理できる
- OEM / fleet / platform側のpredictive diagnosticsへ、操舵系contextを渡す具体的な接点がある
- repair feedback loopの少なくとも一部に接続できる可能性がある

Hold:

- Bosch公開情報は強いが、steering-specific featureの需要が未確認
- EPSサプライヤ内の既存診断・品質・service資料との重複が未確認
- OEMやfleet platformとの接続がまだ仮説

Stop候補:

- 操舵系featureが汎用CAN/telematicsや一般DTC説明と区別できない
...
| EPSサプライヤのビジネスに転記できるか | 仮説としては可能。feature readinessと診断境界のassessmentに落とす必要がある。 | Medium | ビジネス仮説、判定に反映 |

## EPSサプライヤとしての言い方

言ってよいこと:

> Boschは2026年に、fleet向けAI-driven predictive maintenanceとvehicle health servicesを強化する動きを公開している。Predictive Diagnosticsでは、connected vehicle data、cloud information、component-specific load and diagnostic features、domain expertise、multiple data sourcesを使い、部品やsystemの状態、故障前兆、整備タイミングを扱う。EPSサプライヤとしては、このような基盤に操舵系を載せるため、操舵負荷feature、診断feature、limit / fallback context、DTC / freeze frame解釈、feedback loop条件を整理できるかを検証する価値がある。
...
> 操舵系featureを渡せば、安全保証、root cause断定、保証費削減ができる。
...
1. 操舵系で、component-specific load featureと言えるものは何か
2. DTC / freeze frame / extended data / software IDのうち、予測診断側へ渡してよいものと、渡すと誤解を生むものは何か
