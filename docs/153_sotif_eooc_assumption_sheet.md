...
| B. 機能不足(functional insufficiency)の観測粒度 | **埋まる。数値で言える**。応答遅れ0.4s / ゲイン15% / バイアス0.05 m/s² を誤検出6.7%の運用点で90%検出 |
...
> 60秒の走行データ1本から、0.4秒級の応答遅れ・15%級のアシスト特性変化・0.05 m/s² の定常オフセットを、誤検出6.7%の運用点で90%検出できる。逆に0.1秒の遅れ・2%のゲイン変化はこの窓長では見えない。
