...
残すのは、EPSサプライヤ内で既存業務との差分を確認する社内レビュー材料だけである。
...
次に進める場合は、外部調査の継続ではなく、EPSサプライヤ内の社内レビューで既存業務との差分を確認する。
...
|---|---|---|---|
| PVC001 | Final: internal review only / no external offer | 使用負荷classの先読みは公開proxyだけでは外販商品にしない | 社内レビューを行う場合だけ、ULC008の2枚sampleを製品企画・診断企画に見せる |
...
| ULC004 | Final: internal review sample only | 荒れた路面 + 操舵は社内レビューsampleのみ | 品質改善・評価企画が既存業務との差分を示す場合だけ再開 |
...
| ULC008 | Final: internal review sample only | 駐車場 + 低速 + 大舵角 + 凹凸は最初の社内レビューsampleのみ | 製品企画と診断企画の両方が具体用途を示す場合だけ再開 |
...
| 故障予測に戻っていないか | Yes。故障、残寿命、交換時期、保証費、安全性はすべて禁止主張へ置いた。 | High | 外販Stop、社内レビュー材料のみとした |
| EPSサプライヤの部署に転記できるか | Partly。製品企画・診断企画は可能性があるが未証明。品質改善・評価企画は二次。 | Low-Medium | 社内レビュー再開条件にした |
...
| 次の判断が可能か | Yes。社内レビュー4問で、完全Archiveか社内限定再開かを切れる。 | Medium | 次アクションを社内レビュー確認にした |

## EPSサプライヤとしての言い方

言ってよいこと:

> 公開proxyだけではEPS内部状態や残寿命は分からない。ただし、低速、大舵角、路面外乱、stop-start、凹凸、通信異常contextを分けると、EPSサプライヤ内の製品企画や診断企画で使える可能性がある。現時点では商品ではなく、既存業務との差分を確認する社内レビュー材料である。
