...
> **量的な形([docs/248](248_false_alarm_tradeoff_results.md))**: 実用の誤検出率(1回/時)で並べると、
> **1秒未満の突発については、車両レベルは床の20倍=約4.2 m/s²(アシスト完全喪失相当)しか捕まらず、
> 部品内部は床の10倍で前駆に2〜30倍の余裕が残る。**
>
> **訂正(2026-09-01, [docs/253](253_persistent_event_results.md))**: 上の一文は**5秒窓・突発事象に限る。**
> 現場の主要な型は分の尺度で持続し([docs/251](251_field_timescale_results.md))、
> **20秒の持続なら車両レベルでも床の1.5倍(約0.31 m/s²)で捕まる。**突発型の 1/14 である。
...
> **この一文を仕様に落とした → [docs/225](225_recorder_specification.md)。**
> 常時武装・5秒以上の窓・不揮発保持・36バイトの個体基準。**主張しないこと7件と空欄6件も仕様の一部である。**
...
- **トリガ設計の最初の入力が公開データから出た**([docs/213](213_trigger_conditions_results.md))。
  **事象はキーサイクルまで保持される形(12.5%)が瞬間的な形(2.8%)より4倍以上多く報告される**。振動・湿気は関連が見えない
- **不安定さは水準に先行しない**([docs/211](211_instability_precursor_results.md))。6デバイス中0で先行。**「不安定さを見る」は主張から外した**
