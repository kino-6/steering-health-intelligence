...
> **重要な追記(2026-08-30, [docs/230](230_inventory_failure_and_igbt.md))**: **棚卸しが漏れていた。**
> 既に使っていたNASAリポジトリに、未取得の電子部品run-to-failureが3件あった(IGBT・Capacitor×2)。
> **IGBTは矩形波ゲート=スイッチ動作**であり、[docs/199](199_pulse_thermal_results.md) が見つけた
> 「素子がスイッチとして動いていない」という欠陥が当てはまらない。
> **「解法に届いていない」の根拠が弱くなった。**docs/192〜199の検証を正しい動作モードでやり直せる。
> **断続故障については変わらない見込み**(未確認)。

## 何があれば動くか

> **要件を13項目に言語化した → [docs/238](238_data_requirement.md)。**
> 各行に、それを決めた失敗が1件ずつ付いている。
> **効くのは2件だけである——故障が出たり消えたりすること、その時刻の正解があること。**
> 残る11件は既存データで近似できている。**最小構成はEPS実機である必要はない。**
