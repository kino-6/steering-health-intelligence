...
| **実用水準(1回/時)では、1秒未満の突発について車両レベルは完全喪失しか捕まらず、部品内部だけが前駆に届く** | **[248](248_false_alarm_tradeoff_results.md)** |
| **持続する事象なら車両レベルでも床の1.5倍(0.31 m/s²)で捕まる。突発型の 1/14**(**訂正**: 上行の結論は5秒窓に限る) | **[253](253_persistent_event_results.md)** |
| **現場の主要な型は分の尺度で始まりイグニッションまで持続する。**622件が明示 | **[251](251_field_timescale_results.md)** |
...
> **訂正(2026-08-31, [docs/239](239_arc_fault_search.md))**: 「断続故障のデータが無い」は不正確だった。
> **アーク故障のデータは存在し、接点開放方式で機構も合致する。**
> **無いのは「断続が起きる前」のデータである。**アークデータは検出器を作るためのもので、事象でトリガされる。
...
**断続故障の公開データは出ない。それを結論の締めにしない。**[AGENTS.md](../AGENTS.md) 0条。

> **決定: 断続故障そのものの実データを待たず、パワーモジュール側の量から仕様を確定させる。**
> **根拠**: 「床がいくらか」は測らないと分からないが、**「床がいくらであれば足りるか」は
> 既に測った前駆の大きさから逆算できる**([docs/260](260_no_data_decisions_protocol.md))。
> 問いの向きを変えれば、同じ空欄が計算可能な設計要件になる。
>
> **したがって R12・R13 は「未達の要件」として残すが、「次アクションを止める理由」にはしない。**
