...
[docs/123](123_steering_predictive_diagnostics_power_monitor_program_question_sheet.md) の照合質問は「サプライヤの対象programで残るか」という形だったため内部資料条件になっていた。
...
> SPD008 power monitorの価値仮説(既存monitorだけでは残らない領域があり、そこに状態説明価値がある)は、**公開情報のみでConfirmed**。内部資料が必要なのは、仮説の検証ではなく**実行**(サプライヤ製品の現行設定確認と部署の使い道確認)だけである。
...
「サプライヤprogramで残るか」は聞いていない。市場に出た他社実例の公開文書だけで判断した。
...
1. 2件の実例は2011〜2017年世代である。**現行世代のprogramではDem設定が改善され、より多くが残る可能性がある**。「サプライヤの現行設定でどうか」だけが残る内部確認点であり、docs/123の質問シートはそのために保存する
...
| 内部資料条件の中身 | 仮説検証+実行 | **実行のみ**(現行世代のサプライヤ設定確認、部署の使い道確認) |
