...
|---|---|---|---|
| **A** | **兆候→故障の橋(EPS内部信号版)** | 実車の故障前後データが公開されていない。6は検査員の目視観察であって内部信号ではない | **経路が見つかった**——NASA PCoEのrun-to-failureデータ(DS015、下記) |
| **B** | **個体基準の有効期間** | 経年・温度で同一個体のUがどれだけ動くか未測定。測ったのは1台の短時間ゆらぎ(最大9.5%)だけ | **同上**。DS015は6個体を健全→故障まで連続追跡している |

**Aが本丸である。** 「見える」ことと「見れば痛みが減る」ことは別であり、後者は仮説のまま。
