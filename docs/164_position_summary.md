...
**NREの実体が具体化した**: program固有の閾値較正 + EOL出荷時基準値工程の設計。
...
| 残る未証明 | 内容 |
|---|---|
| ~~**A''**~~ | ~~frettingが単調な観測量を生むか~~ → **公開情報では答えられない。問いを閉じた**([docs/175](175_close_contact_question.md))。公開劣化データ110件の総覧に接点は0件。文献の3段階カーブから「**fretting系に実用的な先行時間はない**」を構成 |
...
- そして**それは調べる対象ではなく、要求する対象である**。第1層はRFQ差別化 + 診断コンテンツNREであり、
  **EOL出荷時基準値工程の仕様は売り物の中身**である。既存工程に何が記録されているかを前提条件にしない
- 桁感: EOL出荷時基準値は既存測定値の要約を数十バイト書き込むだけであり、タクトタイムを増やさない。
  追加ラインも新規センサも要らない
...
## 故障族ごとの主張(2026-08-23、[docs/175](175_close_contact_question.md))

**一つの族で実証したことを別の族へ流用しない。**

| 故障族 | 先行の実証 | 部品側の価値 | 市場での多さ([docs/171](171_eps_wearout_mechanism_scan.md)) |
|---|---|---|---|
| 熱・接合部の劣化 | **あり**(故障の1〜2段階前、ノイズの20〜300倍) | 早期の状態説明 | **低い**(熱語は平均の0.54倍) |
| 接点のfretting | **水準では取れない**。ただし**劣化の窓は存在する**(断続・回復・再発、公開リコールに明記)。発生率として定量できるかは未検証([docs/176](176_correction_degradation_window.md)) | event時点の文脈説明 + 再発頻度 | **高い**(電気症状は2.81倍) |
| 巻線の短絡 | 段階的severityのみ実測 | 状態説明 | — |

**市場で最も多い族が、最も先行時間を取りにくい。** 本研究に不利な事実であり、そのまま置く。

## 事業判断(2026-08-24、[docs/182](182_business_judgment_update.md))

**「成立する」とは主張しない**(変わらず)。ただし理由を更新した。

- 旧: 「有利かタダかは商談の中でしか答えが出ない」→ 閉じる
- 新: **問いを「この種類の機能は有償で残る側か」に置き換えれば公開情報で答えられる。現時点は有償側**
  - 競合が名前を付けて製品化・量産投入(無償の当然機能なら、そうしない)
  - 規制は**結果**(監視・報告・ライフタイム管理)を義務化し、**方法**(何を測るか)を規定していない
  - 法規が**機能**を指定するとタダ化する(ESC / TPMS / eCall)。**結果**だけの指定なら方法で差別化余地が残る

**誤り条件と点検結果**:
- ①部品レベルの劣化指標の規格化 → **一部踏まれた**([docs/183](183_autosar_capability_channel.md))。AUTOSARは宛先(STR capability)を標準化済み。劣化からの導出則は未規定
- ②OEMが同一要件を全サプライヤに課す → 未点検
- ③競合が無償バンドル化する → 未点検
- ④ADS普及が量として足りない → **西側では踏まれた**([docs/184](184_ads_deployment_reality.md))。Mercedes/BMW/HondaがL3を停止・終了。中国のみ指定区間・50〜80km/hで商用展開

**したがって需要の主たる根拠を、SOTIF/ADS規制から「OEMの品質・保証の経済性」(ADS非依存)へ戻す。**
SOTIFは乗れる枠として維持するが、需要の柱にしない。

## 誤診の痛みの所在(2026-08-25、[docs/185](185_misdiagnosis_quantified.md))

苦情160万件で定量した。**族を限定しない主張は落とす。**

| 主張 | データ |
|---|---|
| ×「EPSは誤診が多い」 | **支持されない**。EPS全体13.79% vs 全苦情13.66%(**1.01倍**)。繰り返し来店はむしろ少ない(0.95倍) |
| ○「**断続的で再現しない故障**は原因不明で終わる率が5倍」 | **支持される**。EPS×断続あり **5.74%** vs 全体基準 1.15%(**5.0倍**)、EPS内で **4.50倍** |

**標的にしてきた族こそが、誤診が集中する族だった。** 再現できないことが痛みであり、部品側の記録だけがその瞬間を持っている。
限界: 言及率であって発生率ではない。EPS×断続ありは383件と母数が小さい。

## 金の筋とSOTIFの筋は別([docs/186](186_how_it_becomes_money.md))

| 用途 | 対象 | 状態 |
|---|---|---|
| **診断証拠**(誤交換を止める) | **全車両** | **今日、規制不要。ここが金の筋** |
| EooC仮定の市場検証 | ADS搭載車のみ | 規制は実在。市場は西側で縮小中 |

**同じ観測が2つに使える、以上の関係は無い。** SOTIFは宣言の様式と将来の枠として維持するが、**需要の柱にしない**。
金額は公開されていないため主張しない。
