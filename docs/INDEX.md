...
- [196 SOTIFに部品側として参加する要素](196_eps_health_element.md): **決定則を要素として組んで動かした。**組んで初めて出た要求——**出荷時基準値は運用動作点範囲を掃引して採ること**(スパン10°Cに対し運用は20〜31°C動き、6個体中1個体が終端で宣言を失った)。個体基準のコストは**36バイト**
- [197 宣言を縛るのは測定ではなく仮定だった](197_what_limits_the_declaration.md): **自己訂正——0.08〜1.0%は分解能であって確度ではない。**Rthが0.16〜2.0%増えるだけで誤差が並ぶ。出荷時基準値の要求を数値化(動作点は出荷時基準値幅の1.7〜3.0倍を動く)
...
- [228 仕様を実車ログの上で動かす 事前登録](228_recorder_simulation_protocol.md): 「記録器がどれだけ黙るか」を測る。出荷時基準値の代用が既知の欠陥を持つことを手順に明記
- [229 1回の走行で採った出荷時基準値では3〜4分の時間を黙る](229_recorder_simulation_results.md): **沈黙 65.6〜80.7%。配備できない。**×5で1%未満に落ちる崖(×2では24〜42%残る)。**ただし×5は同一走行内の話で、走行をまたぐ要求はdocs/227の7〜35倍。**出力は1件24バイトで容量は問題にならない
- [226 仕様の空欄を2つ埋める 事前登録](226_filling_spec_blanks_protocol.md): 6欄中2欄は手元のデータで埋まる。分位点を結果を見る前に固定
- [227 空欄2つが埋まった](227_filling_spec_blanks_results.md): **出荷時基準値は1走行では速度軸で7〜35倍足りない**(実車2,495ログ)。**記録は397日(13か月)生きる必要がある**(車検2,234万台のp90)。事前バッファは設計で消えた。**残る空欄は4件**
- [225 記録器の仕様](225_recorder_specification.md): **docs/188の一文を実装可能な仕様にした。**新データ無し、全項目に根拠付き。**トリガは条件で絞らない**(公正な分母では2.0倍超が1件も無い)、窓5秒以上、不揮発必須、出荷時基準値36バイト。**主張してはならない7件**と**空欄6件**を仕様の一部として明記
...
- [240 突発事象を数える 事前登録](240_excursion_event_protocol.md): docs/174は窓の中央値とIQRで測っていた。**両方とも上位0.1%の突発を消す。**観測量だけ変える
- [241 突発事象も増えなかった／閾値が成立しない](241_excursion_event_results.md): **K1は2/13で不成立。**加えて**抵抗が整数量子化されMADが潰れ、8個体で最大発生率＝基準窓の値**。閾値は作り直さない。**SOReDDを2観測量で閉じた**
- **[211 訂正済]** 「不安定さは水準に先行しない」は**半導体のドリフトについての結果**。接点系では文献が逆を述べており**未検定**(公開データが無い)。主張には戻さない
- [242 総覧の付録表をようやく読んだ](242_survey_appendix_read.md): **4回目で読めた。110件中、intermittent/connector/contact/relay/solder/fretting はすべて0件。**トランジスタ2件は既に持っているNASAの2件。**ただし総覧自体にSOReDD・KAIST・comma・インバータの4件の穴があり、「0件」は不在の証明にならない**
- [243 断続事象を注入して検出力を測る 事前登録](243_intermittent_injection_protocol.md): docs/225の空欄を「R13が無い」で閉じたのは早すぎた。**注入すれば正解時刻は構成上わかる**
- [244 空欄が1つ埋まった／検出器は2つ要る](244_intermittent_injection_results.md): **誤検出1.0%/5秒窓で、1秒未満は床の10倍・2秒は床の5倍。****短い事象は最大値型のみ、長い事象は平均型が2倍鋭い**——docs/240の見立ては半分だけ正しかった。**車両レベルの値でありECU内部信号ではない**
- [245 部品内部で同じ注入をする 事前登録](245_internal_signal_injection_protocol.md): 前駆の大きさ(床の20〜300倍)と検出限界を同じ単位で並べる
- [246 部品内部では床の2〜3倍で足りる](246_internal_signal_injection_results.md): **6個体中5で2〜3倍。車両レベル(10倍)より3〜5倍鋭い。****前駆は20〜300倍なので2〜150倍の余裕。**部品の中で見る利得が初めて数値になった。ただし**大きさの比較であって断続故障が捕まる証明ではない**
- [247 誤検出を実用水準まで下げる 事前登録](247_false_alarm_tradeoff_protocol.md): docs/244の1.0%/窓は約7回/時で配備できない。標本の外の水準を先に表に書いた
- [248 実用水準では車両レベルが破綻する](248_false_alarm_tradeoff_results.md): **誤検出1回/時にすると、車両レベルは床の20倍=約4.2 m/s²=「アシスト完全喪失」しか捕まらない。****部品内部は床の10倍で、前駆(20〜300倍)に2〜30倍の余裕が残る。**docs/188の主張が同じ土俵の数値になった
- [249 EooCシートを通しで監査](249_eooc_sheet_audit.md): **5行が古いまま「埋まる」と書いてあった。**capability値が成立すると信じていた時期の行。**成果物が根拠より強い主張をしていた。**訂正し、**同じ腐り方をcheck_repoで機械的に塞いだ**
