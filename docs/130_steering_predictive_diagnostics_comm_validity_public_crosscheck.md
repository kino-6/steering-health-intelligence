...
docs/124の照合項目(依存signalのvalidity、fallback近接、既存DTCで足りるか)を、「自社programでどうか」ではなく「市場に出た実車両の実務でどうだったか」で判定した。
...
2. 残る内部条件(実行のみ): 現行自社設定の確認(docs/123質問シート)、IdsM役割分担、2部署以上の使い道、SOTIF KQ1
