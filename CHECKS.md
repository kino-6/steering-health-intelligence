...
`.git/hooks/pre-commit` から自動実行される。**ブロッキング13件、警告1件。**
...
| **spec coverage** | [docs/225](docs/225_recorder_specification.md) に行を足して、実装・測定値・見送りのどれにも分類しないこと | 仕様が38行に伸びる間、実装は6本のスクリプトに散っており、**両者が同じものを指しているかを誰も見ていなかった**([docs/288](docs/288_spec_coverage.md)) |
| **forbidden output** | 要素の欄名に、主張しないと決めた語(能力値・予測・箇所・余寿命)が入ること | 7項目は文章だった。**取り下げた能力値を計算するコードが残った**([TROUBLES.md](TROUBLES.md) T23)のは、文章がコードを止められなかったため |
...
| *(警告)* plain japanese | [AGENTS.md](AGENTS.md) 3.1 の凝った言い回し | 2026-09-07 ユーザ指摘「レポートの記述が気持ち悪すぎる」「どうやったら検知、修正できる？ Gate 設置は難しそうだが」。検出器は [scripts/check_plain_ja.py](scripts/check_plain_ja.py)。**3 段に分けてある**——(1) 常に誤りの文字列（「——」・ルール0の禁止語）、(2) 数えられる性質（1文90文字超）、(3) 誤検出のある推定（体言止め・特定の言い回し）。(3) の誤検出率は `--measure` で実測して決めた（散文行の 3.8%、抜き取り検査で本物）。既存文書に 1,696 件あるため警告。**報告文は `check_report.py` が全段ブロックする**（積み残しが無いため） |
