...
| `scripts/check_report_style.py` | **描いた後の絵を検査する。**文字のはみ出し・重なり・大きさ、明暗両モードのコントラスト |

**`check_repo.py` が `reports/*.json` を毎コミット検査する。**
図の `source`（実測 / 仮想 / 置いた値 / 計算）が無いと描かせない。
描いた座標が viewBox からはみ出しても止まる。
**検査は 2 回ある。**JSON の検査は書いた内容を見て、描画後の検査は描かれた絵を見る。
文字が箱に入るか、2 つのラベルが重なるか、暗いモードで読めるかは、**描くまで分からない。**
